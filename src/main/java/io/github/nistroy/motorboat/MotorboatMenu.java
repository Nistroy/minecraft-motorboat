...
/**
 * Menu d'une barque : slot carburant (n'accepte qu'un combustible de four) plus un coffre de 27.
 *
...
public class MotorboatMenu extends AbstractContainerMenu {
    ...
    public ItemStack quickMoveStack(Player player, int index) {
        ...
        int playerStart = MotorboatEntity.CONTAINER_SIZE;
        if (index < playerStart) {
            if (!moveItemStackTo(stack, playerStart, slots.size(), true)) {
                return ItemStack.EMPTY;
            }
        } else if (AbstractFurnaceBlockEntity.isFuel(stack) && moveItemStackTo(stack, 0, 1, false)) {
            // rien de plus : le combustible part en priorité dans le réservoir
        } else if (!moveItemStackTo(stack, MotorboatEntity.FIRST_STORAGE_SLOT, playerStart, false)) {
            return ItemStack.EMPTY;
        }
        if (stack.isEmpty()) {
